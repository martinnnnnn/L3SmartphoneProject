    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mMap.setOnInfoWindowClickListener(new GoogleMap.OnInfoWindowClickListener() {
            ...
            public void onInfoWindowClick(Marker arg0) {
                ...
                final EditText input = new EditText(ViewTwo.this);

                input.setInputType(InputType.TYPE_CLASS_NUMBER);
                input.setText(Integer.toString(allMarkersMap.get(arg0).getSpeed()));
                input.setSelection(input.getText().length());

                final Button deleteBtn = new Button(ViewTwo.this);
                deleteBtn.setText("Supprimer");
                deleteBtn.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        int id = allMarkersMap.get(m).getId();
                        m.hideInfoWindow();
                        points.remove(id);
                        allMarkersMap.remove(m);
                        m.remove();
                        //Changement des id suivants
                        Waypoint value;
                        for (Map.Entry<Marker, Waypoint> entry : allMarkersMap.entrySet()) {
                ...
                LinearLayout lnl = new LinearLayout(ViewTwo.this);
                lnl.setOrientation(LinearLayout.VERTICAL);
                lnl.setLayoutParams(lp);
                lnl.addView(input);
                lnl.addView(deleteBtn);

                AlertDialog.Builder alertDialog = new AlertDialog.Builder(ViewTwo.this);
